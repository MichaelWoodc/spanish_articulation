...
function updateBatchWithTranscription(batchNumber, transcriptionData, originalWords) {
  ...
  console.log(`Estimated timing - Total: ${totalDuration.toFixed(2)}s, Avg per word: ${avgWordDuration.toFixed(2)}s`);
  ...
  allTranscribedWords.forEach((transcribedWord, index) => {
    ...
    if (transcribedWord.start > expectedEndTime && currentGroup.length > 0) {
      wordGroups.push({
        expectedWord: originalWords[currentGroupIndex] || "—",
  ...
  wordGroups.forEach((group, index) => {
    const transcribedText = group.transcribedWords.map(w => w.word).join(' ') || "—";
    ...
    const isCorrect = !hasH && transcribedText !== "—";
    ...
    row.innerHTML = `
      ...
      <td class="time-cell">${group.start ? group.start.toFixed(2) + "s" : "—"}</td>
      <td class="time-cell">${group.end ? group.end.toFixed(2) + "s" : "—"}</td>
      <td class="time-cell">${avgConfidence !== "N/A" ? (avgConfidence * 100).toFixed(1) + "%" : "—"}</td>
    `;
